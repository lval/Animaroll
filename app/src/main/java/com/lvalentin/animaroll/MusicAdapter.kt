...
class MusicAdapter(private val dataList: List<MusicVm>): RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    var onItemCheck: ((MusicVm) -> Unit)? = null
    ...
    class ItemViewHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
        val chkAlbum: CheckBox = itemView.findViewById(R.id.chk_music)
        val tvAlbumName: TextView = itemView.findViewById(R.id.tv_music_name)
        val tvAlbumItemCnt: TextView = itemView.findViewById(R.id.tv_music_item_cnt)
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            Enums.RecyclerViewType.HEADER.id -> {
                val view = LayoutInflater.from(parent.context).inflate(R.layout.music_header, parent, false)
